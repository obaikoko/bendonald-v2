...
const getAllStudents = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (keyword) {
      whereClause.OR = [
        { firstName: { contains: keyword as string, mode: "insensitive" } },
        { lastName: { contains: keyword as string, mode: "insensitive" } },
        { otherName: { contains: keyword as string, mode: "insensitive" } },
      ];
    }
    ...
    // SubLevel (A,B,C)
    if (subLevel) {
      const arms = (subLevel as string).split(",");

      whereClause.subLevel = {
        in: arms,
      };
    }
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const updateStudent = await prisma.student.update({
      ...
      data: {
        ...
        studentId: schoolId ?? student.studentId,
