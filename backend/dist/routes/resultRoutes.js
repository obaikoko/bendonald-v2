...
router.route("/positions").post(authMiddleware_1.protect, authMiddleware_1.admin, resultController_1.generatePositions);
router.route("/broadsheet").post(authMiddleware_1.protect, resultController_1.generateBroadsheet);
router.route("/payment").put(authMiddleware_1.protect, authMiddleware_1.admin, resultController_1.updateResultPayment);
router
    .route("/subjects")
    .put(authMiddleware_1.protect, authMiddleware_1.admin, resultController_1.manualSubjectRemoval)
    .post(authMiddleware_1.protect, authMiddleware_1.admin, resultController_1.addSubjectToResults);
router.route("/data").get(authMiddleware_1.protect, authMiddleware_1.admin, resultController_1.resultData);
router.route("/data/student-results").get(authMiddleware_1.protect, resultController_1.studentResultData);
router.route("/pdf").get(authMiddleware_1.protect, authMiddleware_1.admin, resultController_1.exportManyResults);
router
    .route("/:id")
